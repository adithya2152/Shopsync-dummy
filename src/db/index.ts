...
import {ShopSync_Categories, ShopSync_Managers, ShopSync_Products, ShopSync_Shops, ShopSync_Users } from "./schema";
...
export async function CreateUser(username: string, email: string, uuid: string, loc: { latitude: number; longitude: number } , role:string) {
    try {
        const now = new Date();
...
export async function RegisterShop(Shopname: string, loc: { latitude: number; longitude: number }, Memail:string, Mauth:string, Mname:string, Mloc: { latitude: number; longitude: number }) {
    try {
        const now = new Date();
        ...
        const [newShop] = await db.insert(ShopSync_Shops).values({
            name: Shopname,
            location: JSON.stringify(loc),
            createdAt: now
        }).returning({ shopId: ShopSync_Shops.id });

        console.log("✅ Shop Successfully Registered:", Shopname, "with ID:", newShop.shopId);
        ...
        if (existingManager.length > 0) {
            console.log("⚠️ Manager email already registered for a different shop:", existingManager[0].authid, existingManager[0].username);
            await db.delete(ShopSync_Shops).where(eq(ShopSync_Shops.id, newShop.shopId));
            console.log("🗑️ Registered shop deleted as manager registration failed");
            return { error: "Manager email already registered. Login to register new shop" };
        }
...
export async function getStoreIDs(managerAuthId: string) {
    try {
        const storeIDs = await db
            ...
            .from(ShopSync_Shops)
            .innerJoin(ShopSync_Managers, eq(ShopSync_Managers.shopId, ShopSync_Shops.id))
...
export async function getCategories()
{
    try {
        ...
        // .orderBy(ShopSync_Categories.name);
...
export async function addInventory(shopId: number, product: { name: string; description: string; price: number; stock: number; mnf_date?: Date; exp_date?: Date; categoryId: number; imgPath?: string }) 
{
    try {
...
export async function updateDiscount(shopId: number, productId: number, discount: number)
{
    try{
        const existingProduct = await db
            .select()
...
export async function updateInventory(shopId: number, product: { id: number, name?: string, description?: string, price?: number, stock?: number, mnf_date?: string, exp_date?: string, categoryId?: number, imgPath?: string }) {
    try {
        ...
        if (product.mnf_date) {
            updateData.mnf_date = new Date(product.mnf_date).toISOString().split("T")[0];
        }
        if (product.exp_date) {
            updateData.exp_date = new Date(product.exp_date).toISOString().split("T")[0];
        }
