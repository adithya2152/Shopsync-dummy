...
import CouponDisplaySection, { Coupon } from "./CouponSection";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const coupons = await axios.get(`/api/coupons?shopId=${shopId}`);
        if (coupons.status === 200 && coupons.data && coupons.data.length > 0) {
          setCoupons(coupons.data);
          console.log("Coupons loaded successfully:", coupons.data);
        }
      } catch (error) {
