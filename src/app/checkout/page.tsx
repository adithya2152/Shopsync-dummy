...
export default function CheckoutPage() {
  ...
  const refreshCart = useCallback(async () => {
    ...
    try {
      ...
      setSubtotal(typeof data.subtotal === "number" ? data.subtotal : 0);
      setPlatformFees(typeof data.platformFees === "number" ? data.platformFees : 0);
      setDiscountAmount(typeof data.discountAmount === "number" ? data.discountAmount : 0);
      setTotal(typeof data.total === "number" ? data.total : 0);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response && err.response.data.errorType === "coupon") {
  ...
  const handleApplyCoupon = async () => {
    let couponResp;
    try {
      couponResp = await axios.get(`/api/coupons?shopId=${shopId}&code=${couponCodeInput}`);
    } catch (error) {
      if(axios.isAxiosError(error) && error.response && error.response.data.errorType === "coupon") {
