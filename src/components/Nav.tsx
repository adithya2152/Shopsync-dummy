...
export default function Nav({ navType }: NavProp) {
  ...
  const debouncedSearch = debounce(async (term: string) => {
    ...
    if (params.shopId) {
      ...
      try {
        const productsRes = await axios.get<{ products: Omit<Product, "type">[] }>(
        ...
        >
                    ...
                    <List>
                      {results.map((item, idx) => (
                        ...
                        > 
                          <ListItemText
                            primary={item.name}
                            secondary={item.type === "Product" ? `${item.type} - ${item.shopName}` : item.type}
                          />
                        </ListItem>
                      ))}
                    </List>
          ...
          {actualNavType !== "landing" && (
            <>
              ...
              >
                <Avatar
                  sx={{ bgcolor: "grey", color: "white", fontWeight: "bold" }}
                />
              </IconButton>
              <Menu
                id="basic-menu"
